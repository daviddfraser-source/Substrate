...
DOC_INCLUDED_TOP_LEVEL = {"docs", "prompts", "templates", "examples", ".governance", "skills", "scripts", "tests"}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        routes = {
            ...
            "/api/e2e/runs": lambda: self.api_e2e_runs(query),
            "/api/e2e/run": lambda: self.api_e2e_run(query.get("id", [""])[0]),
            "/api/break-fix/items": lambda: self.api_break_fix_items(query),
            "/api/break-fix/item": lambda: self.api_break_fix_item(query.get("id", [""])[0]),
            "/api/break-fix/summary": self.api_break_fix_summary,
    ...
    def do_POST(self):
        ...
        routes = {
            ...
            "/api/break-fix/open": lambda: self.api_break_fix_open(body),
            "/api/break-fix/start": lambda: self.api_break_fix_start(body),
            "/api/break-fix/resolve": lambda: self.api_break_fix_resolve(body),
            "/api/break-fix/reject": lambda: self.api_break_fix_reject(body),
            "/api/break-fix/note": lambda: self.api_break_fix_note(body),
    ...
    def api_status(self) -> Dict:
        ...
        state = load_state()
        break_fix = break_fix_summary(BREAK_FIX_LOG)
        active_by_packet = break_fix.get("by_packet_active", {})
        ...
        for area in defn.get("work_areas", []):
            ...
            for p in defn.get("packets", []):
                if p.get("area_id") == area["id"]:
                    ...
                    active_break_fixes = int(active_by_packet.get(p["id"], 0))
                    pkts.append({
                        ...
                        "notes": ps.get("notes"),
                        "break_fix_active": active_break_fixes,
                    })
            areas.append({
                "id": area["id"],
                "title": area["title"],
    ...
    def _load_e2e_store(self) -> Dict:
        """Load persisted e2e run records, returning safe defaults when absent."""
        if not E2E_RUNS.exists():
            return {"schema_version": "1.0", "updated_at": None, "runs": []}
        with E2E_RUNS.open(encoding="utf-8") as f:
            data = json.load(f)
    ...
    def api_e2e_runs(self, query: Dict[str, List[str]]) -> Dict:
        """Return recent e2e run summaries with optional filters."""
        try:
        ...
        suite_filter = (query.get("suite", [""])[0] or "").strip().lower()
        packet_filter = (query.get("packet_id", [""])[0] or "").strip()

        store = self._load_e2e_store()
        runs = []
        for run in store.get("runs", []):
            if status_filter and str(run.get("status", "")).lower() != status_filter:
                continue
            if suite_filter and str(run.get("suite", "")).lower() != suite_filter:
                continue
            if packet_filter and str(run.get("packet_id", "")) != packet_filter:
                continue
            runs.append(
                {
        ...
        return {
            "success": True,
            "schema_version": store.get("schema_version", "1.0"),
    ...
    def api_e2e_run(self, run_id: str) -> Dict:
        """Return full details for a specific e2e run id."""
        run_id = (run_id or "").strip()
        if not run_id:
            return {"success": False, "message": "Missing id"}

        store = self._load_e2e_store()
        run = next((item for item in store.get("runs", []) if item.get("run_id") == run_id), None)
        if not run:
            return {"success": False, "message": f"E2E run not found: {run_id}"}
        return {"success": True, "run": run, "schema_version": store.get("schema_version", "1.0")}

    def _parse_text_list(self, value) -> List[str]:
        if value is None:
            return []
        if isinstance(value, list):
            source = value
        elif isinstance(value, str):
            source = [token.strip() for token in value.split(",")]
        else:
            return []
        return [str(token).strip() for token in source if str(token).strip()]

    def api_break_fix_summary(self) -> Dict:
        """Return aggregate break-fix summary for dashboard/reporting."""
        return {"success": True, "summary": break_fix_summary(BREAK_FIX_LOG)}

    def api_break_fix_items(self, query: Dict[str, List[str]]) -> Dict:
        """Return break-fix queue with optional status/severity/packet/owner filters."""
        try:
        ...
        packet_id = (query.get("packet_id", [""])[0] or "").strip()
        ...
        try:
            ...
            return {"success": True, "items": rows, "returned": len(rows), "summary": break_fix_summary(BREAK_FIX_LOG)}
        except ValueError as e:
            return {"success": False, "message": str(e)}

    def api_break_fix_item(self, fix_id: str) -> Dict:
        """Return full break-fix item details."""
        fix_id = (fix_id or "").strip()
        if not fix_id:
            return {"success": False, "message": "Missing id"}
        item = get_break_fix(BREAK_FIX_LOG, fix_id)
        if not item:
            return {"success": False, "message": f"Break-fix item not found: {fix_id}"}
        return {"success": True, "item": item}

    def api_break_fix_open(self, body: Dict) -> Dict:
        actor = str(body.get("actor") or body.get("agent_name") or "").strip()
        title = str(body.get("title") or "").strip()
        description = str(body.get("description") or "").strip()
        if not actor or not title or not description:
            return {"success": False, "message": "Missing actor, title, or description"}
        try:
            fix_id = open_break_fix(
                ...
                linked_packets=self._parse_text_list(body.get("linked_packets")),
                findings=self._parse_text_list(body.get("findings")),
                evidence=self._parse_text_list(body.get("evidence")),
            )
            return {"success": True, "fix_id": fix_id, "item": get_break_fix(BREAK_FIX_LOG, fix_id)}
        except ValueError as e:
            return {"success": False, "message": str(e)}

    def api_break_fix_start(self, body: Dict) -> Dict:
        fix_id = str(body.get("fix_id") or body.get("id") or "").strip()
        actor = str(body.get("actor") or body.get("agent_name") or "").strip()
        if not fix_id or not actor:
            return {"success": False, "message": "Missing fix_id or actor"}
        try:
            ok, msg = start_break_fix(
                BREAK_FIX_LOG,
                fix_id=fix_id,
                actor=actor,
                owner=str(body.get("owner") or ""),
                note=str(body.get("note") or body.get("notes") or ""),
            )
            return {"success": ok, "message": msg, "item": get_break_fix(BREAK_FIX_LOG, fix_id) if ok else None}
        except ValueError as e:
            return {"success": False, "message": str(e)}

    def api_break_fix_resolve(self, body: Dict) -> Dict:
        fix_id = str(body.get("fix_id") or body.get("id") or "").strip()
        actor = str(body.get("actor") or body.get("agent_name") or "").strip()
        summary = str(body.get("resolution_summary") or body.get("summary") or "").strip()
        evidence = self._parse_text_list(body.get("evidence"))
        if not fix_id or not actor:
            return {"success": False, "message": "Missing fix_id or actor"}
        if not summary:
            return {"success": False, "message": "Missing resolution_summary"}
        if not evidence:
            return {"success": False, "message": "Resolve requires evidence"}
        try:
    ...
    def api_break_fix_reject(self, body: Dict) -> Dict:
        fix_id = str(body.get("fix_id") or body.get("id") or "").strip()
        actor = str(body.get("actor") or body.get("agent_name") or "").strip()
        reason = str(body.get("reason") or "").strip()
        if not fix_id or not actor or not reason:
            return {"success": False, "message": "Missing fix_id, actor, or reason"}
        try:
            ok, msg = reject_break_fix(
                BREAK_FIX_LOG,
                fix_id=fix_id,
                actor=actor,
                reason=reason,
    ...
    def api_break_fix_note(self, body: Dict) -> Dict:
        fix_id = str(body.get("fix_id") or body.get("id") or "").strip()
        actor = str(body.get("actor") or body.get("agent_name") or "").strip()
        note = str(body.get("note") or body.get("notes") or "").strip()
        if not fix_id or not actor or not note:
            return {"success": False, "message": "Missing fix_id, actor, or note"}
        try:
    ...
    def api_packet(self, packet_id: str) -> Dict:
        ...
        linked_break_fixes = list_break_fixes(BREAK_FIX_LOG, packet_id=packet_id, limit=200)
        active_break_fix_count = sum(1 for item in linked_break_fixes if item.get("status") in {"open", "in_progress"})
        ...
        return {
            ...
            "packet": {
                ...
                "break_fix_active": active_break_fix_count,
            },
            "documents": docs,
            "break_fix_items": linked_break_fixes,
            "events": events[-50:],
        }
