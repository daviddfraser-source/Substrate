...
START_ENTRY=""
if [[ -f "${ROOT_DIR}/start.py" ]]; then
  START_ENTRY="${ROOT_DIR}/start.py"
elif [[ -f "${ROOT_DIR}/../start.py" ]]; then
  START_ENTRY="${ROOT_DIR}/../start.py"
fi
...
check_cli_compile() {
  ...
  python3 -m py_compile .governance/wbs_cli.py .governance/wbs_server.py "${START_ENTRY}"
}
...
check_cmd "CLI/server/start compile" check_cli_compile
