...
function TreeItem({ node, depth, onNodeClick }: { node: TreeNode; depth: number; onNodeClick?: (id: string) => void }) {
  ...
  return (
    <div>
      <div
        className="flex items-center gap-2 py-1.5 px-2 rounded-[var(--radius-md)] hover:bg-[var(--bg-inset)] cursor-pointer text-sm"
        style={{ paddingLeft: `${12 + depth * 20}px` }}
        onClick={() => onNodeClick?.(node.id)}
      >
        <button
          className="w-4 text-xs text-[var(--text-tertiary)]"
...
export function TreeView({ nodes, onNodeClick }: TreeViewProps) {
  return (
    <div className="border border-[var(--border-default)] rounded-[var(--radius-xl)] bg-[var(--bg-primary)] p-2">
